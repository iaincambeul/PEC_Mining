...
def contact_maker(Fd_ind, chain_atoms, chain_dict, res_list):
  ...
  print(len(chain_atoms[0]))
  print(len(chain_atoms[1]))
  ind = 0
  nonFdAtoms = []
  ...
  """ind = 1
  for item in fdResPartnerObjects:
    if item != []:
      print item 
    else:
      print "Empty"
    print ind
    ind += 1"""
  #This reads out correctly, something is wrong later on
...
def object_parser(partnerContacts, ferredoxinRes, chain_dict):
  ...
  print(splitCleanedBothRes)
  print(len(splitCleanedBothRes))
